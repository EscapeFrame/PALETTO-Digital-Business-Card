...
export default function BusinessCard3D({ member, isPreview = false, showDownloadButton = false }: BusinessCard3DProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const dataUrl = await toJpeg(frontCardRef.current, {
        quality: 1,
        pixelRatio: 3,
        cacheBust: true,
        ...
        filter: (node) => {
          // 다운로드 버튼 제외
          if (node.classList && node.classList.contains('download-button')) {
            return false;
          }
          return true;
        },
        style: {
          transform: 'none', // 3D 효과 제거하고 평면으로 캡처
        }
      });
      
      const link = document.createElement('a');
      link.download = `${member.name}-business-card.jpg`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
      ...
      >
        ...
        >
          ...
          <div className="relative h-full p-5 flex justify-between">
            {/* Download Icon - Absolute Positioned */}
            <div 
              role="button"
              onClick={(e) => {
                e.preventDefault();
              ...
              className="download-button absolute top-4 right-4 p-2 text-gray-300 hover:text-[#2196F3] transition-colors z-20 cursor-pointer"
              title="명함 다운로드"
            >
            ...
            <div className="flex flex-col justify-center">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  <svg className="w-3.5 h-3.5 text-[#2196F3]" fill="currentColor" viewBox="0 0 24 24">
                ...
                {member.social.github && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-600">{member.social.github}</span>
                  </div>
                )}
                ...
                {member.social.instagram && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-600">{member.social.instagram}</span>
                  </div>
                )}
                ...
                {member.social.discord && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-600">{member.social.discord}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        >
          ...
          <svg className="absolute right-3 top-0 h-full" width="2" viewBox="0 0 2 351" fill="none" preserveAspectRatio="none">
            <path d="M0 1C0 0.447715 0.447715 0 1 0C1.55228 0 2 0.447715 2 1H1H0ZM1 351H0V1H1H2V351H1Z" fill="url(#paint0_linear_back)"/>
            <defs>
              <linearGradient id="paint0_linear_back" x1="0.5" y1="351" x2="0.5" y2="1" gradientUnits="userSpaceOnUse">
                <stop stopColor="#0077FF"/>
                <stop offset="1" stopColor="#0077FF" stopOpacity="0"/>
              </linearGradient>
            </defs>
          ...
          <div className="h-full flex">
            {/* Left Vertical Line */}
            <div className="w-1 h-full bg-gradient-to-b from-[#2196F3] via-[#64B5F6] to-[#2196F3]" />
